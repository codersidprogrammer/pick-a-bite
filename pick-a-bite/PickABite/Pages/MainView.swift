...
// 1. Define a custom environment key
struct FromWidgetKey: EnvironmentKey {
    static var defaultValue: Bool = false
}

// 2. Extend the EnvironmentValues to include your custom key
extension EnvironmentValues {
    var fromWidget: Bool {
        get { self[FromWidgetKey.self] }
        set { self[FromWidgetKey.self] = newValue }
    }
}
...
struct MainView: View {
    ...
    @StateObject private var rouletteService: RoulettePageViewModel<UserHistoryRepository> = {
    ...
    @State private var isFromWidget: Bool = false
        ...
        NavigationStack(path: $path) {
            VStack {
                ...
                .navigationDestination(isPresented: $isGotoRouletteView) {
                    ...
                    .environmentObject(rouletteService)
                    .environment(\.fromWidget, isFromWidget)
                }
            }
            .background(
                LinearGradient(
    ...
    func handleDeepLink(_ url: URL) {
        Logger.log("Receiving deep link: \(url)")
            // Check if the URL scheme matches your deep link
            if url.scheme == "pickabite" && url.host == "roulettePage" {
                isGotoRouletteView = true
                isFromWidget = true
                Logger.log("Navigating to Roulette Page")
            }
